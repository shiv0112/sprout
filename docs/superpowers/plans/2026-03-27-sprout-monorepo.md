# Sprout Monorepo Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Transform the existing Babel prototype into a production-grade, cloud-native SaaS monorepo ("Sprout") — a self-evolving tool registry for autonomous AI agents with MCP protocol support, 5 independently deployable microservices, Firebase Auth, PostgreSQL, and two React frontends.
...
**Reference:** `docs/Preliminary-1.pdf` — Preliminary Report
...
sprout/
├── packages/
│   └── core/                         # sprout-core Python library
│       ├── pyproject.toml
│       ├── src/sprout_core/
│       │   ├── __init__.py
│       │   ├── spec.py               # BabelToolSpec → SproutToolSpec
...
│       │   ├── schema.json           # Sprout tool JSON Schema
│       │   └── compiler/
│       │       ├── __init__.py
│       │       ├── base.py
│       │       ├── ag2.py
...
This chunk sets up the monorepo scaffolding and migrates the existing Babel core into a proper `sprout-core` Python package. By the end, `sprout-core` is installable, tested, and the existing tool registry works with the new package.
...
Run: `cd /Users/pinetortoise/Desktop/sprout && cat pyproject.toml && cat pnpm-workspace.yaml`
...
### Task 2: Create sprout-core Python package (migrate babel_registry core)

**Files:**
- Create: `packages/core/pyproject.toml`
- Create: `packages/core/src/sprout_core/__init__.py`
- Create: `packages/core/src/sprout_core/spec.py` (from `babel_registry/spec.py`)
- Create: `packages/core/src/sprout_core/registry.py` (abstract interface)
- Create: `packages/core/src/sprout_core/loader.py` (from `babel_registry/loader.py`)
- Create: `packages/core/src/sprout_core/runtime.py` (from `babel_registry/runtime.py`)
- Create: `packages/core/src/sprout_core/schema.json` (from `babel_registry/spec/babel.schema.json`)
- Create: `packages/core/src/sprout_core/compiler/__init__.py`
- Create: `packages/core/src/sprout_core/compiler/base.py`
- Create: `packages/core/src/sprout_core/compiler/ag2.py`
- Create: `packages/core/src/sprout_core/compiler/langchain.py`
- Create: `packages/core/src/sprout_core/compiler/mistral.py`
- Create: `packages/core/src/sprout_core/compiler/pydantic_ai.py`

- [ ] **Step 1: Create sprout-core pyproject.toml**
...
name = "sprout-core"
version = "0.1.0"
description = "Core library for Sprout: tool specs, registry interface, framework compilers"
requires-python = ">=3.13"
dependencies = [
    "pydantic>=2.0",
    "pyyaml>=6.0",
...
all = ["sprout-core[ag2,langchain,mistral,pydantic-ai]"]
...
packages = ["src/sprout_core"]
```

- [ ] **Step 2: Migrate spec.py — rename Babel→Sprout, modernize with Pydantic v2**

Port `babel_registry/spec.py` to `packages/core/src/sprout_core/spec.py`:
- Rename `BabelToolSpec` → `SproutToolSpec`, `BabelTool` → `SproutTool`
- Convert dataclasses to Pydantic BaseModel for validation
- Keep `@sprout_tool` decorator (renamed from `@babel_tool`)
...
class SproutToolSpec(BaseModel):
    id: str
    name: str
    description: str
...
`packages/core/src/sprout_core/registry.py`:
```python
from abc import ABC, abstractmethod
from .spec import SproutTool
...
class ToolRegistry(ABC):
    ...
    @abstractmethod
    def search(self, query: str) -> list[SproutTool]: ...
    @abstractmethod
...
Port `babel_registry/loader.py` → `packages/core/src/sprout_core/loader.py`:
- Update all `Babel*` references to `Sprout*`
...
Port `babel_registry/runtime.py` → `packages/core/src/sprout_core/runtime.py`
Port `babel_registry/compiler/*.py` → `packages/core/src/sprout_core/compiler/*.py`
...
`packages/core/src/sprout_core/pg_registry.py`:
```python
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select, text
from .spec import SproutTool, SproutToolSpec
...
class PostgresRegistry(ToolRegistry):
    ...
    async def register(self, tool: SproutTool) -> None: ...
    async def get(self, tool_id: str) -> SproutTool | None: ...
    async def search(self, query: str) -> list[SproutTool]: ...
    async def list_all(self) -> list[SproutTool]: ...
...
```bash
cp babel_registry/spec/babel.schema.json packages/core/src/sprout_core/schema.json
```

- [ ] **Step 7: Create __init__.py with public API**

```python
# packages/core/src/sprout_core/__init__.py
from .spec import SproutTool, SproutToolSpec, ToolParam, ToolReturn, sprout_tool
from .registry import ToolRegistry, InMemoryRegistry
from .loader import SproutLoader
from .runtime import SproutRuntime

__all__ = [
    "SproutTool", "SproutToolSpec", "ToolParam", "ToolReturn", "sprout_tool",
    "ToolRegistry", "InMemoryRegistry",
    "SproutLoader", "SproutRuntime",
]
```

- [ ] **Step 8: Write tests for sprout-core**

Create `packages/core/tests/test_spec.py`:
- Port relevant tests from `test_babel.py`
- Test SproutToolSpec validation, ToolParam types, @sprout_tool decorator
...
git commit -m "feat: create sprout-core package — migrate babel_registry core with Pydantic v2"
```
...
- Update: tool namespace from `com.aria.tools.*` → `com.sprout.tools.*`
...
Replace `com.aria.tools.` with `com.sprout.tools.` in every spec.yaml and directory name.

- [ ] **Step 3: Rename tool directories**

```bash
cd tools && for dir in com.aria.tools.*/; do
  new_name="${dir/com.aria.tools./com.sprout.tools.}"
  mv "$dir" "$new_name"
done
```

- [ ] **Step 4: Verify tool loading with sprout-core**

```python
from sprout_core import SproutLoader
...
git add tools/
git rm -r registry/
git commit -m "feat: migrate tool registry — rename namespace to com.sprout.tools"
```
...
GRANT ALL PRIVILEGES ON DATABASE sprout_registry TO sprout;
GRANT ALL PRIVILEGES ON DATABASE sprout_chat TO sprout;
...
Replace with new Sprout monorepo README covering the new architecture, tech stack, setup instructions.
...
# Firebase
FIREBASE_PROJECT_ID=sprout-dev

# Anthropic (for Synthesis + Chat Backend)
ANTHROPIC_API_KEY=sk-ant-...

# Registry API
SPROUT_REGISTRY_DATABASE_URL=postgresql+asyncpg://sprout:sprout_dev@localhost:5432/sprout_registry
SPROUT_REGISTRY_GCS_BUCKET=sprout-tool-artifacts
SPROUT_REGISTRY_TOOL_EXECUTOR_URL=http://localhost:8400

# Chat Backend
SPROUT_CHAT_DATABASE_URL=postgresql+asyncpg://sprout:sprout_dev@localhost:5432/sprout_chat
SPROUT_CHAT_REGISTRY_API_URL=http://localhost:8100
SPROUT_CHAT_TOOL_EXECUTOR_URL=http://localhost:8400
SPROUT_CHAT_VAULT_ENCRYPTION_KEY=<generate with: python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())">

# Synthesis
SPROUT_SYNTHESIS_GCP_PROJECT_ID=sprout-dev
SPROUT_SYNTHESIS_REGISTRY_API_URL=http://localhost:8100

# MCP Server
SPROUT_MCP_REGISTRY_API_URL=http://localhost:8100
```

- [ ] **Step 2: Create sprout-core test conftest.py**

```python
# packages/core/tests/conftest.py
import pytest
from sprout_core import InMemoryRegistry, SproutTool, SproutToolSpec, ToolParam, ToolReturn
...
def sample_tool(sample_spec):
    ...
    return SproutTool(spec=sample_spec, fn=hello)
```
...
name = "registry-api"
version = "0.1.0"
description = "Sprout Registry API — tool CRUD, search, RBAC, semver"
requires-python = ">=3.13"
dependencies = [
    "sprout-core",
    "fastapi>=0.115",
...
class Settings(BaseSettings):
    database_url: str = "postgresql+asyncpg://sprout:sprout_dev@localhost:5432/sprout_registry"
    gcs_bucket: str = "sprout-tool-artifacts"
    firebase_project_id: str = "sprout-dev"
    tool_executor_url: str = "http://localhost:8400"
    cors_origins: list[str] = ["http://localhost:5173", "http://localhost:5174"]
    log_level: str = "INFO"

    model_config = {"env_prefix": "SPROUT_REGISTRY_"}
...
class Namespace(Base):
    ...
    id: Mapped[str] = mapped_column(String(128), primary_key=True)  # e.g. "com.sprout.tools"
...
class Tool(Base):
    __tablename__ = "tools"
    id: Mapped[str] = mapped_column(String(256), primary_key=True)  # com.sprout.tools.weather
...
async def test_create_namespace_and_tool(db_session):
    ns = Namespace(id="com.sprout.test", owner_uid="uid123", display_name="Test")
    db_session.add(ns)
    tool = Tool(
        id="com.sprout.test.hello",
        namespace_id="com.sprout.test",
        name="hello",
        description="Says hello",
        spec_json={"id": "com.sprout.test.hello"},
    )
    db_session.add(tool)
    await db_session.commit()
    assert (await db_session.get(Tool, "com.sprout.test.hello")) is not None
```
...
name = "tool-executor"
version = "0.1.0"
description = "Sprout Tool Executor — gVisor-sandboxed tool execution"
requires-python = ">=3.13"
dependencies = [
    "sprout-core",
    "fastapi>=0.115",
...
name = "synthesis"
version = "0.1.0"
description = "Sprout Synthesis Service — Claude API tool generation + Pub/Sub consumer"
requires-python = ">=3.13"
dependencies = [
    "sprout-core",
    "fastapi>=0.115",
...
class ToolGenerator:
    ...
    def _build_prompt(self, request: SynthesisRequest) -> str:
        return f"""Generate a Sprout tool with the following specification:
...
1. spec.yaml — following the Sprout tool spec format
...
SYNTHESIS_SYSTEM_PROMPT = """You are a tool synthesis engine for the Sprout platform.
You generate Python tools that conform to the Sprout spec format.
...
name = "chat-backend"
version = "0.1.0"
description = "Sprout Chat Backend — LLM Planning Agent, Graph Executor, SSE streaming"
requires-python = ">=3.13"
dependencies = [
    "sprout-core",
    "fastapi>=0.115",
...
class GraphExecutor:
    """Executes task graphs using Claude API tool-use + Sprout Tool Executor."""
...
This is a key differentiator — the MCP Server exposes the entire Sprout tool registry to any MCP-compatible AI client (Claude Desktop, Cursor, VS Code Copilot, Windsurf).
...
name = "mcp-server"
version = "0.1.0"
description = "Sprout MCP Server — JSON-RPC adapter bridging MCP protocol to Registry API"
requires-python = ">=3.13"
dependencies = [
    "fastapi>=0.115",
    "uvicorn[standard]>=0.34",
...
class McpHandlers:
        ...
        match method:
            case "initialize":
                return make_response(req_id, {
                    ...
                    "serverInfo": {"name": "sprout", "version": "0.1.0"},
                })
            case "tools/list":
    ...
    def _to_mcp_tool(self, tool: dict) -> dict:
        """Convert Sprout tool spec to MCP tool format."""
        return {
            "name": tool["name"],
...
app = FastAPI(title="Sprout MCP Server")
...
class RegistryClient:
    """HTTP client for Sprout Registry API."""
...
resource "google_cloud_run_v2_service" "services" {
  ...
  name     = "sprout-${each.key}"
  location = var.region

  template {
    containers {
      image = "${var.region}-docker.pkg.dev/${var.project_id}/sprout/${each.key}:latest"
...
jobs:
  ...
  test-python:
    ...
    services:
      postgres:
        image: postgres:16-alpine
        env: { POSTGRES_USER: sprout, POSTGRES_PASSWORD: test, POSTGRES_DB: sprout_test }
        ports: ["5432:5432"]
    steps:
      - uses: actions/checkout@v4
      - uses: astral-sh/setup-uv@v5
    ...
    steps:
      ...
      - run: |
          gcloud builds submit \
            --tag ${{ vars.REGION }}-docker.pkg.dev/${{ vars.PROJECT_ID }}/sprout/${{ matrix.service }}:${{ github.sha }} \
            -f services/${{ matrix.service }}/Dockerfile .
          gcloud run deploy sprout-${{ matrix.service }} \
            --image ${{ vars.REGION }}-docker.pkg.dev/${{ vars.PROJECT_ID }}/sprout/${{ matrix.service }}:${{ github.sha }} \
            --region ${{ vars.REGION }}
```
...
- Create: `packages/core/src/sprout_core/telemetry.py`

- [ ] **Step 1: Create shared telemetry setup**

```python
# packages/core/src/sprout_core/telemetry.py
...
```bash
git add packages/core/src/sprout_core/telemetry.py
...
New README covering:
- What Sprout is (from PDF executive summary)
...
```bash
git add -A
git commit -m "feat: add integration tests + update README for Sprout monorepo"
```
...
|----------------|-----------|-----------|
| Registry API    | 8100      | sprout-registry-api |
| Chat Backend    | 8200      | sprout-chat-backend |
| MCP Server      | 8300      | sprout-mcp-server |
| Tool Executor   | 8400      | sprout-tool-executor |
| Synthesis       | 8500      | sprout-synthesis |
| Chat UI         | 5173      | Firebase Hosting (chat.sprout.dev) |
| Registry UI     | 5174      | Firebase Hosting (registry.sprout.dev) |
...
1. **Tasks 1-5** (Chunk 1): Monorepo + core library = working `sprout-core` package
