...
Sprout's API endpoints are currently unprotected. This spec adds Clerk-based authentication to protect write operations while keeping read endpoints public for tool discovery.
...
| Auth module location | `sprout_shared/auth.py` | Shared across all services, single source of truth |
...
Browser request:
  Authorization: Bearer <clerk-jwt>
    → sprout_shared.auth.require_auth
    → verify JWT against Clerk JWKS (cached)
    → return SproutUser

CLI/MCP request:
  X-API-Key: sprout_<user_id>_<random>
    → sprout_shared.auth.require_auth
...
### `packages/shared/sprout_shared/auth.py`
...
async def require_auth(request: Request) -> SproutUser:
    """FastAPI dependency. Returns SproutUser or raises 401."""
    # 1. Check Authorization: Bearer <jwt>
    # 2. Fallback to X-API-Key: <key>
    # 3. Raise HTTPException(401) if neither valid

async def require_jwt_auth(request: Request) -> SproutUser:
...
- API key format: `sprout_<user_id>_<32-char-hex>` — user_id is embedded in the key
...
**New dependencies for `sprout-shared`:** `pyjwt[crypto]`, `httpx`

### `packages/shared/sprout_shared/config.py`
...
SPROUT_INTERNAL_SECRET: str   # from env, for service-to-service auth
```
...
| `POST /synthesis/callback` | internal (validate `SPROUT_INTERNAL_SECRET` header) |
...
| Route | Auth |
|---|---|
| `POST /sprout/start` | `require_auth` |
| `POST /sprout/execute/{run_id}` | `require_auth` |
| `GET /sprout/stream/{run_id}` | public (auth checked at /start) |

The `SproutUser` from auth is logged with the run for traceability but runs are not scoped per-user in this phase.
...
- Registry API validates the header value matches its own `SPROUT_INTERNAL_SECRET` env var
- No Clerk involvement for internal calls

**Required changes to synthesis_service:**
- Add `SPROUT_INTERNAL_SECRET` as a direct env var (not prefixed with `SPROUT_SYNTHESIS_` to avoid awkward naming)
- Update `sprout_synthesis/callback.py` `notify_success()` and `notify_failure()` to include the `X-Internal-Secret` header
- Add `SPROUT_INTERNAL_SECRET` to `sprout_synthesis/config.py` Settings class with `env_prefix` override or as a standalone field
...
| `CLERK_DOMAIN` | registry_api, chat_backend (via sprout_shared) | JWKS URL for JWT verification |
| `CLERK_SECRET_KEY` | registry_api, chat_backend (via sprout_shared) | Clerk Backend API calls |
| `SPROUT_INTERNAL_SECRET` | registry_api, synthesis_service | Service-to-service callback auth |
...
- SSE streaming (auth checked at /sprout/start, not on stream)
...
7. Synthesis callback validates `SPROUT_INTERNAL_SECRET`
