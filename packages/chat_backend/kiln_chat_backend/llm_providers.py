...
Env vars:
  ...
  KILN_NIM_MODEL        NIM model id            (default: z-ai/glm-5.2)
  KILN_MISTRAL_MODEL    Mistral model id        (default: mistral-large-latest)
  KILN_LLM_PRIMARY      "nim" | "mistral"       (default: nim)
"""
...
def _nim_model() -> str:
    return os.environ.get("KILN_NIM_MODEL", DEFAULT_NIM_MODEL).strip() or DEFAULT_NIM_MODEL


def _mistral_model() -> str:
    return os.environ.get("KILN_MISTRAL_MODEL", DEFAULT_MISTRAL_MODEL).strip() or DEFAULT_MISTRAL_MODEL
...
def provider_chain(mistral_api_key: str = "") -> list[dict]:
    ...
    primary = os.environ.get("KILN_LLM_PRIMARY", "nim").strip().lower()
