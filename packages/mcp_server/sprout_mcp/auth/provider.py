...
def _signing_secret() -> bytes:
    configured = os.environ.get("SPROUT_INTERNAL_SECRET", "").strip()
    if configured:
...
class SproutAuthorizationCode(AuthorizationCode):
    user_id: str
...
class SproutOAuthProvider:
    ...
    async def authorize(
        ...
        ui_base = os.environ.get("SPROUT_UI_URL", "").rstrip("/")
        if ui_base:
            return (
                f"{ui_base}/mcp-auth?"
    ...
    ) -> SproutAuthorizationCode | None:
        data = self._store.get_auth_code(authorization_code)
        if data is None:
            return None
        if data.get("client_id") != client.client_id:
            return None
        return SproutAuthorizationCode(**data)

    async def exchange_authorization_code(
        self,
        client: OAuthClientInformationFull,
        authorization_code: SproutAuthorizationCode,
    ) -> OAuthToken:
        ...
        access_token = SproutAccessToken(
        ...
        refresh_token = SproutRefreshToken(
    ...
    async def load_access_token(self, token: str) -> SproutAccessToken | None:
        data = self._store.get_access_token(token)
        if data is None:
            return None
        return SproutAccessToken(**data)

    async def load_refresh_token(
        self, client: OAuthClientInformationFull, refresh_token: str
    ) -> SproutRefreshToken | None:
        data = self._store.get_refresh_token(refresh_token)
        if data is None:
            return None
        if data.get("client_id") != client.client_id:
            return None
        return SproutRefreshToken(**data)
    ...
    async def exchange_refresh_token(
        ...
        new_refresh_token = SproutRefreshToken(
            token=new_refresh,
            client_id=client.client_id,
            scopes=scopes or refresh_token.scopes,
            user_id=refresh_token.user_id,
    ...
    async def revoke_token(self, token: SproutAccessToken | SproutRefreshToken) -> None:
        if isinstance(token, SproutAccessToken):
        ...
        elif isinstance(token, SproutRefreshToken):
