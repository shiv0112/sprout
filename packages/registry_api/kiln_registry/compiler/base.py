"""
kiln_registry/compiler/base.py
──────────────────────────────
Abstract base class for all framework adapters.

Every adapter receives a KilnTool and must return
a framework-native object. Nothing else is shared
between adapters — they're completely independent.

To add a new framework:
    1. Create kiln_registry/compiler/my_framework.py
    2. Subclass BaseAdapter
    3. Implement compile()
    4. Register in kiln_registry/runtime.py ADAPTERS dict
...
class BaseAdapter(ABC):
    ...
    @abstractmethod
    def compile(self, tool: KilnTool) -> Any:
        """
        Convert a KilnTool into a native framework object.

        Args:
            tool: The KilnTool from the registry.
    ...
    def _build_json_schema(self, tool: KilnTool) -> dict:
        """
