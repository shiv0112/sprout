"""
kiln_registry/sqlite_registry.py
─────────────────────────────────
SQLite-backed drop-in replacement for KilnRegistry.
...
Limitation (by design):
  ...
  reload their callables — exactly what `import kiln_registry.tools.core_tools`
  already does.
...
def _spec_to_dict(spec: KilnToolSpec) -> dict:
    return {
        "id":          spec.id,
        "name":        spec.name,
...
def _dict_to_spec(d: dict) -> KilnToolSpec:
    return KilnToolSpec(
        id=d["id"],
        name=d["name"],
...
class SQLiteRegistry:
    """
    SQLite-backed Kiln tool registry.

    Identical public interface to KilnRegistry — swap it in by changing
    one line in registry.py. No adapter, runtime, or tool code changes.

    Usage:
        registry = SQLiteRegistry("kiln_registry.db")
        registry.register(my_tool)
        tool = registry.get("com.kiln.tools.weather")
    """

    def __init__(self, db_path: str = "kiln_registry.db"):
    ...
    def _row_to_tool(self, row) -> KilnTool | None:
        """Rebuild a KilnTool from a DB row. Returns None if callable not loaded."""
        spec = _dict_to_spec(json.loads(row[0]))
        fn   = self._fns.get(spec.id)
        if fn is None:
            return None
        return KilnTool(spec=spec, fn=fn)
