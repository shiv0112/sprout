"""CLI entrypoints for sprout_registry.

Exposed as the ``sprout-registry`` console script so k8s Jobs and init-containers
can run migrations without invoking Alembic directly::

    sprout-registry migrate              # upgrade head
    sprout-registry migrate --revision X # upgrade to specific revision
    sprout-registry migrate --downgrade  # downgrade one step
...
def _alembic_config():
    ...
    the ``sprout_registry`` package on the path.
    """
    try:
        ...
        raise RuntimeError(
            ...
            "Install with: pip install 'sprout-registry-api[server]'"
        ) from e

    migrations_dir = resources.files("sprout_registry").joinpath("migrations")
    cfg = Config()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="sprout-registry")
