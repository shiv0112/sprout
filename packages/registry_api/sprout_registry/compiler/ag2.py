"""
sprout_registry/compiler/ag2.py
─────────────────────────────
Compiles a SproutTool into AG2-compatible objects.
...
so the caller just does:
    ...
    compiled = runtime.get("com.sprout.tools.weather", target="ag2")
    register_function(
        compiled.fn,
        caller=assistant,
        executor=executor,
...
class AG2Adapter(BaseAdapter):
    ...
    def compile(self, tool: SproutTool) -> CompiledAG2Tool:
        """
        ...
        # Build a wrapper that enforces the Sprout spec's parameter contract
...
def _make_ag2_wrapper(tool: SproutTool) -> Callable:
    """
