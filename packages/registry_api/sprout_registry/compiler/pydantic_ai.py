"""
sprout_registry/compiler/pydantic_ai.py
─────────────────────────────
Compiles a SproutTool into a format usable with Pydantic AI.
...
Usage:
    compiled = runtime.get("com.sprout.tools.weather", target="pydantic_ai")
    agent = Agent("openai:gpt-4o", tools=[compiled])

    Or access the raw wrapped function:
    compiled_fn = runtime.get("com.sprout.tools.weather", target="pydantic_ai")
"""
...
class PydanticAIAdapter(BaseAdapter):
    ...
    def compile(self, tool: SproutTool) -> CompiledPydanticAITool:
        """
...
def _make_pydantic_ai_wrapper(tool: SproutTool) -> Callable:
    """
