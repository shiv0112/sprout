"""
sprout_registry/integrations.py
─────────────────────────────
Generate copy-pasteable code snippets that embed any registered Sprout tool
...
where only `sprout-registry-api` (no extras) is installed.

Returned shape per target:
    {
        "target":            "langchain",
        "label":             "LangChain",
        "install":           'pip install "sprout-registry-api[langchain]"',
        "language":          "python",
...
def _arg_schema(tool: SproutTool) -> dict[str, Any]:
...
def _example_args(tool: SproutTool) -> dict[str, Any]:
...
def _curl(tool: SproutTool, registry_url: str) -> str:
    ...
    return (
        ...
        f"  -H 'Authorization: Bearer YOUR_SPROUT_API_KEY' \\\n"
        f"  -d '{args_escaped}'"
    )


def _python_requests(tool: SproutTool, registry_url: str) -> str:
    ...
    return (
        ...
        f'    headers={{"Authorization": f"Bearer {{os.environ[\'SPROUT_API_KEY\']}}"}},\n'
        f'    json={{"args": {args}}},\n'
        f'    timeout=30,\n'
        f")\n"
...
def _ag2(tool: SproutTool, registry_url: str) -> str:
    return (
        "# Requires:  pip install \"sprout-registry-api[ag2]\"\n"
        "from sprout_registry.runtime import SproutRuntime\n"
        "from autogen import AssistantAgent, UserProxyAgent\n\n"
        f'runtime = SproutRuntime(target="ag2")\n'
...
def _langchain(tool: SproutTool, registry_url: str) -> str:
    return (
        "# Requires:  pip install \"sprout-registry-api[langchain]\"\n"
        "from sprout_registry.runtime import SproutRuntime\n"
        "from langchain.agents import AgentExecutor, create_tool_calling_agent\n"
        "from langchain_openai import ChatOpenAI\n"
        "from langchain_core.prompts import ChatPromptTemplate\n\n"
        f'runtime = SproutRuntime(target="langchain")\n'
...
def _pydantic_ai(tool: SproutTool, registry_url: str) -> str:
    return (
        "# Requires:  pip install \"sprout-registry-api[pydantic_ai]\"\n"
        "from sprout_registry.runtime import SproutRuntime\n"
        "from pydantic_ai import Agent\n\n"
        f'runtime = SproutRuntime(target="pydantic_ai")\n'
        f'tool = runtime.get("{tool.id}")\n\n'
...
def _mistral(tool: SproutTool, registry_url: str) -> str:
...
def _openai(tool: SproutTool, registry_url: str) -> str:
    schema = _arg_schema(tool)
    schema_text = json.dumps(schema, indent=2).replace("\n", "\n    ")
    return (
...
def _mcp(tool: SproutTool, registry_url: str, mcp_url: str) -> str:
    config = {
        "mcpServers": {
            "sprout": {
                "transport": "streamable-http",
                "url": f"{mcp_url}/mcp",
                "headers": {"Authorization": "Bearer YOUR_SPROUT_API_KEY"},
            }
        }
    }
...
def integrations_for(
    ...
    return [
        ...
        },
        {
            ...
            "label": "AG2 (AutoGen)",
            "language": "python",
            "install": 'pip install "sprout-registry-api[ag2]"',
        ...
        },
        {
            ...
            "label": "LangChain",
            "language": "python",
            "install": 'pip install "sprout-registry-api[langchain]" langchain langchain-openai',
        ...
        },
        {
            ...
            "label": "Pydantic AI",
            "language": "python",
            "install": 'pip install "sprout-registry-api[pydantic_ai]"',
