"""
sprout_registry/loader.py
───────────────────────
SproutLoader: load a tool from a spec.yaml + implementation.py on disk.

This is the entry point for synthesis-generated tools.
The synthesis pipeline writes two files into the registry directory:

    spec.yaml          <- validated against sprout.schema.json
    <entrypoint>.py    <- pure Python, no framework imports

SproutLoader reads both, validates the spec, dynamically imports the
function, runs the test fixtures, and registers the result in the
SQLite registry — producing the same SproutTool object that the
@sprout_tool decorator produces for hand-written tools.
...
The pipeline:
    ...
    spec.yaml + implementation.py
          ...
          |  register in SQLite
          |
        SproutTool  ->  any adapter  ->  any framework

Usage:
    loader = SproutLoader()

    # Load & register one versioned tool
    tool = loader.load("registry/tools/com.sprout.tools.weather/1.0.0")

    # Run test fixtures without registering
    report = loader.test("registry/tools/com.sprout.tools.weather/1.0.0")
...
_SCHEMA_PATH = Path(sprout_shared.__file__).parent / "schema" / "sprout.schema.json"

# ── YAML type -> Sprout type string ─────────────────────────────────────────────
...
class SproutLoader:
    """
    Loads SproutTools from spec.yaml + implementation.py on disk.

    Identical output to @sprout_tool — both produce a SproutTool that the
    ...
    def load(self, tool_dir: str | Path) -> SproutTool:
        """
        ...
        Args:
            tool_dir: Versioned directory, e.g.
                      registry/tools/com.sprout.tools.weather/1.0.0

        Returns:
            A SproutTool registered in the SQLite registry and ready for
    ...
    def load_all(self, registry_dir: str | Path) -> list[SproutTool]:
        """
        Scan a registry directory and load the latest version of every tool.

        Expected layout:
            registry_dir/
                com.sprout.tools.weather/
                    1.0.0/
                        spec.yaml
                        weather.py
                com.sprout.tools.resy_booking/
                    1.0.0/
                        spec.yaml
                        resy_booking.py
    ...
    def _read_spec(self, tool_dir: Path) -> dict:
        ...
        # Accept both sprout_version and babel_version for backwards compatibility
        # during migration from Babel to Sprout.
        if "babel_version" in raw and "sprout_version" not in raw:
            raw["sprout_version"] = raw["babel_version"]
    ...
    def _build_spec(self, raw: dict) -> SproutToolSpec:
        """Convert parsed YAML dict -> SproutToolSpec (same type all adapters use)."""
        tool_meta = raw["tool"]
        iface     = raw["interface"]
