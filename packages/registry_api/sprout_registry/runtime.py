"""
sprout_registry/runtime.py
────────────────────────
SproutRuntime: the main entry point for consuming tools.
...
Usage:
    ...
    runtime = SproutRuntime(target="ag2")

    # Load a tool — returns native AG2 CompiledAG2Tool
    tool = runtime.get("com.sprout.tools.weather")
    tool.register(caller=assistant, executor=executor)

    # Switch target on the fly
    lc_tool = runtime.get("com.sprout.tools.weather", target="langchain")
...
from .registry import SproutRegistry, get_global_registry
...
class SproutRuntime:
    """
    Loads tools from a SproutRegistry and compiles them
    ...
    def __init__(
        self,
        target: str = "ag2",
        registry: SproutRegistry | None = None,
    ):
    ...
    def get(self, tool_id: str, target: str | None = None) -> Any:
        ...
        Args:
            tool_id: Full tool ID, e.g. "com.sprout.tools.weather"
        ...
        sprout_tool = self._registry.get(tool_id)
        if sprout_tool is None:
            sprout_tool = self._registry.query(tool_id)
        if sprout_tool is None:
            raise KeyError(
        ...
        return self._compile(sprout_tool, target)
    ...
    def _compile(self, tool: SproutTool, target: str) -> Any:
