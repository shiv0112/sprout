...
Switch with the ``KILN_LOG_FORMAT`` environment variable.
...
``X-Kiln-Request-ID`` set by the ``KilnRequestIDMiddleware``.
...
from kiln_shared.request_id import RequestIDLoggingFilter
...
def setup_logging(level: str = "INFO") -> None:
    ...
    log_format = os.environ.get("KILN_LOG_FORMAT", "text").lower()
