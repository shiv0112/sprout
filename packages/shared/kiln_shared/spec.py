...
kiln/spec.py
─────────────
The heart of Kiln. A KilnTool is a plain Python function
plus a KilnToolSpec that describes it in a framework-agnostic way.
...
@dataclass
class ToolParam:
    """One input parameter of a Kiln tool."""
    name: str
...
class KilnToolSpec:
    ...
    """
    id: str                             # e.g. "com.kiln.tools.weather"
...
class KilnTool:
    """
    A registered Kiln tool: spec metadata + the actual callable.
...
def kiln_tool(
    ...
    """
    Decorator that wraps a plain Python function as a KilnTool.
    ...
    def decorator(fn: Callable) -> KilnTool:
...
def _python_type_to_str(t: Any) -> str:
    """Map Python type → Kiln type string."""
    mapping = {
        str: "str",
        int: "int",
        float: "float",
