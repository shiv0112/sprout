...
sprout_shared.auth
----------------
FastAPI authentication dependencies for Sprout services.

Supports two auth methods:
  1. Clerk JWT (Authorization: Bearer <token>) — for browser clients
  2. API key (X-API-Key: sprout_<user_id>_<hex>) — for CLI/MCP clients

Usage in FastAPI routes:
    from sprout_shared.auth import require_auth, require_jwt_auth, SproutUser

    @app.post("/tools/register")
    async def register_tool(user: SproutUser = Depends(require_auth)):
...
_api_key_cache: dict[str, tuple[SproutUser, float]] = {}
...
async def _verify_jwt(token: str) -> SproutUser:
...
async def _verify_api_key(api_key: str) -> SproutUser:
    """
    Verify an API key by extracting the user_id from the key format
    (sprout_<user_id>_<hex>) and confirming against Clerk user metadata.
    """
    config = get_config()
    ...
    # Parse key format: sprout_<user_id>_<32-char-hex>
    # user_id may contain underscores (e.g., user_2abc123), so we extract
    # the last 32 chars as hex and everything between "sprout_" and the hex as user_id
    if not api_key.startswith("sprout_") or len(api_key) < 38:  # "sprout_" + at least 1 char + "_" + 32 hex
    ...
    hex_part = api_key[-32:]
    middle = api_key[5:-32]  # everything between "sprout_" and the hex
...
async def require_auth(request: Request) -> SproutUser:
    ...
    Side effect: stores the resolved ``SproutUser`` on ``request.state.user``
    so downstream middleware (e.g. ``sprout_shared.rate_limit.sprout_user_key``)
    can read it without re-running the auth chain.
    """
    user: SproutUser | None = None
        ...
        if api_key:
            ...
            if internal_secret:
                ...
                if config.internal_secret and internal_secret == config.internal_secret:
                    user = SproutUser(user_id="internal", email="", name="internal-service")
