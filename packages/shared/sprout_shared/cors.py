...
sprout_shared.cors
----------------
CORS middleware installer shared across all Sprout FastAPI services.
...
3. **Production enforcement** — if ``SPROUT_ENV=production`` and
...
4. **Dev-friendly default** — when ``SPROUT_ENV`` is unset or "development",
...
# Methods every Sprout service needs. Kept explicit so adding a new one
...
# default hides custom headers). Exposing X-Sprout-Request-ID lets the
...
def resolve_cors_origins() -> list[str]:
    """Return the active CORS allowlist, honoring env vars + SPROUT_ENV.
    ...
    env = os.environ.get("SPROUT_ENV", "development").lower()
    if env in {"production", "prod"}:
        raise RuntimeError(
            "CORS_ORIGINS must be set when SPROUT_ENV=production. "
...
def install_cors(
    ...
    *,
    ...
    """Add the Sprout CORS middleware to a FastAPI app with strict defaults.
