"""
sprout_shared.httpx_client
------------------------
httpx client factories that auto-attach the current Sprout request ID to
every outbound request, so the correlation chain set up by
``SproutRequestIDMiddleware`` extends across service boundaries.
...
The factory:
  ...
  1. Reads the current request ID from the contextvar (set by the
     inbound ``SproutRequestIDMiddleware``).
  2. Adds it as the ``X-Sprout-Request-ID`` request header before send.
  3. Honors any caller-supplied header — if you explicitly pass
     ``headers={"X-Sprout-Request-ID": "..."}`` that wins.
...
Both async and sync flavors are exported because some Sprout code paths
...
from sprout_shared.request_id import REQUEST_ID_HEADER, current_request_id
