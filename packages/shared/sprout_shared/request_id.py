"""
sprout_shared.request_id
----------------------
Per-request correlation IDs for distributed tracing.

Adds an ``X-Sprout-Request-ID`` header to every request/response and binds
...
``sprout_shared.logging_config`` automatically picks up the contextvar
...
# Matches the header used by curl/clients across the Sprout stack.
REQUEST_ID_HEADER = "X-Sprout-Request-ID"

# Default value when there's no active request (background tasks, scripts).
_DEFAULT = "-"

_request_id_ctx: ContextVar[str] = ContextVar("sprout_request_id", default=_DEFAULT)
...
class SproutRequestIDMiddleware(BaseHTTPMiddleware):
    """ASGI middleware: assign + propagate ``X-Sprout-Request-ID``.
