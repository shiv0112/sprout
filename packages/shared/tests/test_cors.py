...
Pin the contract:
  ...
  - KILN_ENV=production WITHOUT CORS_ORIGINS raises at install time
  - The installed middleware exposes X-Kiln-Request-ID on responses
"""
...
from kiln_shared.cors import install_cors, resolve_cors_origins
...
def test_resolve_origins_honors_explicit_env_var(
    ...
    monkeypatch.setenv("CORS_ORIGINS", "https://kiln.example,https://other.example")
...
def test_resolve_origins_prod_with_explicit_config_passes(
    ...
    monkeypatch.setenv("KILN_ENV", "production")
    monkeypatch.setenv("CORS_ORIGINS", "https://kiln.example")
    assert resolve_cors_origins() == ["https://kiln.example"]


def test_install_cors_exposes_request_id_header(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Browser JS must be able to read X-Kiln-Request-ID off every response.
    ...
    assert "x-kiln-request-id" in expose
