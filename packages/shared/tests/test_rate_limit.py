...
def test_sprout_user_key_uses_user_id_when_authenticated() -> None:
    req = _request_with_user(_StubUser(user_id="clerk_user_abc"))
    assert sprout_user_key(req) == "user:clerk_user_abc"


def test_sprout_user_key_falls_back_to_ip_when_anonymous() -> None:
    req = _request_with_user(None)
    key = sprout_user_key(req)
...
async def test_handler_widens_unknown_exception_to_500() -> None:
    ...
    req = _request_with_user(None)
    resp = await sprout_rate_limit_exceeded_handler(req, ValueError("boom"))
...
def test_integration_route_blocks_after_threshold(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("SPROUT_RATE_LIMIT_DEFAULT", "2/minute")
    monkeypatch.setenv("SPROUT_RATE_LIMIT_STORAGE_URI", "memory://")
