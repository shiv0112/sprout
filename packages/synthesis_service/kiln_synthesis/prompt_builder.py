...
def write_context(workspace: Path, request: SynthesizeRequest) -> None:
    ...
    context = f"""\
# Kiln Tool Synthesis Context

## Your Task

Generate a Kiln tool with the following requirements:

- **Tool name**: {request.tool_name}
- **Tool ID**: com.kiln.tools.{request.tool_name}
...
1. **spec.yaml** — Valid Kiln tool spec following the format above
...
def build_prompt(workspace: Path, request: SynthesizeRequest) -> str:
    ...
    return (
        ...
        f"Generate a Kiln tool called '{request.tool_name}' that: {request.description}. "
